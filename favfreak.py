...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(
            description='FavFreak - a Favicon Hash based asset mapper')
        parser.add_argument('-o', '--output', help='Output file name')
        parser.add_argument(
            '--shodan', help='Prints Shodan Dorks', action='store_true')
        parser.add_argument('-on', '--orgname',
                            help='Enter the name of the target org')
    ...
    |       |     |   \/   |       |    \u001b[31m\_ |______\u001b[0m\u001b[32m |     | |    \_
        ...
        for i in a.keys():
            if i in fingerprint.keys():
                print("\u001b[31m["+fingerprint[i]+"] \u001b[0m" +
                      str(i) + " - count : " + str(len(a[i])))
               # print('\n'.join(a[i][:-12]))
                if len(a[i]) > 0:
                    for k in a[i]:
                        print("     " + k[:-12])
        ...
        if args.shodan:
            ...
            for i in a.keys():
                if i != 0:
                    if args.orgname:
                        print(
                            f"\u001b[34m[DORK]\u001b[0m org:\"{args.orgname}\" http.favicon.hash:"+str(i))
                    else:
                        print(
                            f"\u001b[34m[DORK]\u001b[0m org:\"Target Name\" http.favicon.hash:"+str(i))
